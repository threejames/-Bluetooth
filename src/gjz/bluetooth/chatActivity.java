...
public class chatActivity extends Activity implements OnItemClickListener,
	...
	// BluetoothAdapter代表本地蓝牙适配器（蓝牙发射器），是所有蓝牙交互的入口。通过它可以搜索其它蓝牙设备，查询已经配对的设备列表，通过已知的MAC地址创建BluetoothDevice，创建BluetoothServerSocket监听来自其它设备的通信。
	...
	public synchronized void onResume() {
		...
		if (Bluetooth.serviceOrCilent == ServerOrCilent.CILENT) {
			...
			if (!address.equals("null")) {// 代表了一个远端的蓝牙设备， 使用它请求远端蓝牙设备连接或者获取
											// 远端蓝牙设备的名称、地址、种类和绑定状态。 （其信息是封装在
											// bluetoothsocket 中）
